...
function OrganizingCommittee() {
      ...
      .then((data) => {
        ...
        const filtered = data.filter(
          (member) =>
            ...
            member.committee_type.trim().toLowerCase() === "organizing committee"
        );

        // Group by sub_committe (main sections)
        const grouped = {};
        ...
        // For THEME CHAIRS: group further by role
        if (grouped["THEME CHAIRS"]) {
          const themeGrouped = {};
          grouped["THEME CHAIRS"].forEach((member) => {
            if (!themeGrouped[member.role]) themeGrouped[member.role] = [];
            themeGrouped[member.role].push(member);
          });
          grouped["THEME CHAIRS"] = themeGrouped;
        }
  ...
  return (
    <>
      ...
      {/* main section */}
      <section className="py-8 bg-gradient-to-b from-gray-50 to-white mb-4">
          ...
          {Object.entries(committee).map(([section, members], idx) => (
            <div key={idx}>
              <h3 className="text-2xl md:text-3xl font-medium text-blue-800 mb-8 border-l-4 border-blue-600 pl-4">
                {section && section.trim() !== "" && section !== "null" ? section : "Members"}
              </h3>

              {section.trim().toUpperCase() === "THEME CHAIRS" ? (
                // Grouped by role for theme chairs
                <div className="space-y-8">
                  {Object.entries(members).map(([role, people], j) => (
                    <div
                      key={j}
                    ...
                    >
                      ...
                      <ul className="list-disc list-inside text-gray-700 space-y-2">
                        {people.map((person, k) => (
                          <li key={k}>
                            <span className=" text-lg font-medium text-gray-900">{person.name}</span>
                            {person.organization && (
                ...
                // Normal view for other committees
...
//   useEffect(() => {
//     fetch("http://localhost:5000/api/organizing-committee")
//       .then((res) => res.json())
//       .then((data) => {
